...
export default function LinkAnalyticsClient({ link, clicks }: LinkAnalyticsClientProps) {
  ...
  const uniqueIPs = new Set(realClicks.map(c => c.ip_address).filter(Boolean)).size;
    ...
    <div className="space-y-6 max-w-[1200px] mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Unique Users", value: uniqueIPs, color: "text-cyan-400", icon: UserCheck },
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><span className="w-4 h-0.5 bg-primary inline-block opacity-50" style={{ borderTop: "2px dashed #7C3AED" }}></span>Total Clicks</span>
